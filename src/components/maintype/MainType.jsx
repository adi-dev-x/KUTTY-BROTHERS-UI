...
import { Search, ChevronLeft, ChevronRight, Layers, Filter } from "lucide-react";
...
const MainTypeDashboard = ({ onLogout }) => {
  ...
  const matchingLines = filteredMainTypes.length;
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h1 className="text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">Main type</h1>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <div className="inline-flex items-center gap-2 rounded-full border border-slate-200/90 bg-gradient-to-b from-white to-slate-50 px-3.5 py-2 text-[11px] shadow-sm ring-1 ring-slate-100">
                      <Layers className="h-3.5 w-3.5 text-amber-600" strokeWidth={2.5} />
                      <span className="text-slate-500">Total</span>
                      <span className="font-bold tabular-nums text-slate-900">{loading ? "—" : mainTypes.length}</span>
                    </div>
                      ...
                      {currentMainTypes.length === 0 ? (
                        ...
                        currentMainTypes.map((m, index) => (
                          <tr
                            ...
                            className={`transition-colors hover:bg-amber-50/60 ${(indexOfFirst + index) % 2 === 1 ? "bg-slate-50/60" : ""}`}
                          >
