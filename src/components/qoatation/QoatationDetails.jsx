...
const QoatationDetails = ({ onLogout }) => {
  const { id } = useParams();
  const navigate = useNavigate();

  const [qoatation, setQoatation] = useState(null);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        setQoatation(res.data?.data ?? null);
      } catch (err) {
        console.error("Error fetching qoatation details:", err);
        setError("Could not load qoatation details.");
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-3 p-3 sm:p-4">
            ...
            {loading ? (
              <div className="flex flex-1 flex-col items-center justify-center">
                ...
                <p className="mt-3 text-sm font-medium text-slate-600">Loading qoatation details…</p>
              </div>
            ) : error ? (
              ...
              <div className="flex flex-1 items-center justify-center">
                <p className="text-sm text-slate-500">No details found for this qoatation.</p>
              </div>
            ) : (
                ...
                <section className="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-100">
                  <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                    ...
                    <span className="text-xs font-semibold text-slate-500">
                      {(qoatation.items || []).length} item{(qoatation.items || []).length !== 1 ? "s" : ""}
                    </span>
                  </div>

                  {(qoatation.items || []).length === 0 ? (
                    <p className="px-4 py-8 text-center text-sm text-slate-400">No items in this qoatation.</p>
                  ) : (
                          ...
                          {(qoatation.items || []).map((it, idx) => (
                            <tr key={idx} className={idx % 2 === 1 ? "bg-slate-50/50" : "bg-white"}>
                              ...
                              <td className="whitespace-nowrap px-4 py-3 text-right font-bold tabular-nums text-amber-800">
                                ₹{Number(qoatation.total_amount || 0).toLocaleString("en-IN")}
                              </td>
                            </tr>
                          ))}
                        </tbody>
...
export default QoatationDetails;
